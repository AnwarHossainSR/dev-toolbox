  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (error.message.includes("No Login Data files")) {
        toast.info("Copy Login Data files to data/ directory", {
          duration: 5000,
        });
      }
    } finally {
      ...
      <div className="flex items-start gap-3 rounded-xl border border-blue-500/30 bg-blue-500/5 px-4 py-3">
        ...
        <div className="text-xs text-blue-700 dark:text-blue-400">
          ...
          <ol className="space-y-0.5 text-[11px] list-decimal list-inside">
            ...
            <li>
              Copy Login Data files to{" "}
              <code className="px-1 py-0.5 rounded bg-muted">
                data/chrome-login-data/
              </code>{" "}
              and{" "}
              <code className="px-1 py-0.5 rounded bg-muted">
                data/edge-login-data/
              </code>
            </li>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-amber-400/30 bg-amber-400/5 px-4 py-3">
        ...
        <div className="text-xs text-amber-600 dark:text-amber-400">
          ...
          <ul className="space-y-0.5 text-[11px]">
            <li>
              • Reads from <strong>copied files only</strong> — original browser
              files untouched
            </li>
      ...
      {mergeResult && mergeResult.conflicts.length > 0 && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
          ...
          <p className="text-xs text-muted-foreground mb-2">
            {mergeResult.stats.conflicts} passwords had different values. Kept
            the latest based on timestamps:
          </p>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                {["URL", "Username", "Password", "Last Updated", "Source"].map(
                  (h) => (
                    <th
                      key={h}
                      className="px-3 py-2.5 text-left font-semibold text-muted-foreground uppercase tracking-wider text-[10px]"
                    >
                      {h}
                    </th>
                  ),
            ...
            <tbody>
              {entries.map((e, i) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 max-w-[150px] truncate text-foreground">
                    {e.username}
                  </td>
                  <td className="px-3 py-2 font-mono text-muted-foreground max-w-[150px] truncate">
                    {showPasswords
                      ? e.password
                      : "•".repeat(Math.min(e.password.length || 8, 12))}
                  </td>
                  <td className="px-3 py-2 text-muted-foreground whitespace-nowrap">
                    {formatDate(e.last_updated)}
                  </td>
      ...
      {entries.length === 0 && !loading && (
        <div className="rounded-xl border border-border bg-muted/30 py-10 text-center">
          <p className="text-sm text-muted-foreground">
            No passwords found. Make sure Login Data files are copied to the
            data/ directory.
          </p>
        </div>
      )}
