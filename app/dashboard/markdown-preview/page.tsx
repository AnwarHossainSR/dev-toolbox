...
function markdownToHtml(markdown: string): string {
  let html = markdown
    .replace(/^### (.*?)$/gm, '<h3 style="font-size: 1.25rem; font-weight: 600; margin-top: 1rem;">$1</h3>')
    .replace(/^## (.*?)$/gm, '<h2 style="font-size: 1.5rem; font-weight: 700; margin-top: 1.5rem;">$1</h2>')
    .replace(/^# (.*?)$/gm, '<h1 style="font-size: 2rem; font-weight: 700; margin-top: 2rem;">$1</h1>')
    .replace(/\*\*(.*?)\*\*/g, '<strong style="font-weight: 600;">$1</strong>')
    .replace(/\*(.*?)\*/g, '<em style="font-style: italic;">$1</em>')
    .replace(/\`(.*?)\`/g, '<code style="background-color: rgba(0,0,0,0.1); padding: 0.2rem 0.4rem; border-radius: 0.25rem; font-family: monospace;">$1</code>')
